...
```bash
composer require soyhuce/laravel-testing
```

You can publish and run the migrations with:

```bash
php artisan vendor:publish --tag="laravel-testing-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="laravel-testing-config"
```
...
```bash
php artisan vendor:publish --tag="laravel-testing-views"
```

## Usage

```php
$testing = new Soyhuce\Testing();
echo $testing->echoPhrase('Hello, Soyhuce!');
```
